      ...
      <va-content style="padding-left: 20px">
        ...
        <div class="row">
          <div class="flex md4 sm6 xs12">
            ...
            <va-slider pins :step="1" :min="1990" :max="2022" track-label-visible v-model="years" @drag-end="onSearch" range>
            </va-slider>
          </div>
        </div>
      </va-content>
...
<script setup lang="ts">
  ...
  import { ref } from "vue";
  const disease = ref('')
  const lineChartDataGenerated = ref({datasets: [{label: 'Nothing to display yet', backgroundColor: 'hsla(220,80%,42%,0.7)', data: []}], labels: []})
  ...
  const onSearch = () => {
    ...
    return bkClient.get("trend", {params: {disease: disease.value, min_yr: years.value[0], max_yr: years.value[1]}}).then(res => {
        ...
        lineChartDataGenerated.value = {datasets: [{label: label, backgroundColor: 'hsla(220,80%,42%,0.7)', data: data}], labels: range};
    })
  }
